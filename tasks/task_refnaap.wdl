...
task refnaap {
  ...
  command <<<
    ...
    # Run the pipeline
    if RefNAAP_CLI.py -i $PWD/reads -o $PWD/refnaap --threads ~{cpu} --MinCov ~{min_coverage} --model ~{model} --Size ~{size} --Right ~{trim_right} --Left ~{trim_left} --TopN ~{top_n_genomes} ~{'--RefFile ' + reference_file};
    then
